...
export default function HomePage() {
  ...
  return (
    <div className="px-4 pt-2 max-w-[640px] mx-auto">
      {/* Header */}
      <div className="text-center mb-7 pt-2 animate-fade-in">
      ...
      <div className="mb-4 animate-fade-in" style={{ animationDelay: "0.1s" }}>
        ...
        <div className="flex flex-col gap-1.5">
          {top3.map((p, i) => (
            <div key={p.id} className="card flex items-center gap-3 !py-3 !px-3.5" style={{ background: medalBg[i], borderLeft: `3px solid ${medalColors[i]}` }}>
              <span className="font-display text-[22px] font-black min-w-[28px]" style={{ color: medalColors[i] }}>{i + 1}</span>
              <div className="flex-1">
                <p className="text-sm font-semibold text-text-warm">{p.name}</p>
                <p className="text-[11px] text-text-muted">@{p.username}</p>
              </div>
              <span className="font-display text-lg font-extrabold" style={{ color: medalColors[i] }}>
                {p.totalPoints} <span className="text-[11px] font-normal">pts</span>
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="mb-4 animate-fade-in" style={{ animationDelay: "0.2s" }}>
        ...
        <div className="card !border-amber-mid/15">
          <p className="text-sm font-semibold mb-3 text-text-warm">{MINI_POLL.title}</p>
          <div className="flex flex-col gap-1.5">
            {MINI_POLL.options.map((opt) => {
              const pct = Math.round(((MINI_POLL.votes[opt] || 0) / totalVotes) * 100);
              const voted = pollVote === opt;
              return (
                <button key={opt} onClick={() => setPollVote(opt)}
                  className="relative flex items-center gap-2.5 py-2.5 px-3 rounded-lg text-left w-full text-sm transition-all"
                  style={{ border: voted ? "1px solid #DFBE38" : "1px solid rgba(255,255,255,0.06)", background: voted ? "rgba(223,190,56,0.08)" : "#07090D" }}>
                  {pollVote && <div className="absolute left-0 top-0 bottom-0 transition-all duration-500" style={{ width: `${pct}%`, background: voted ? "rgba(223,190,56,0.1)" : "rgba(255,255,255,0.03)" }} />}
                  <span className="relative flex-1" style={{ fontWeight: voted ? 600 : 400 }}>{opt}</span>
                  {pollVote && <span className="relative text-xs text-text-muted font-semibold">{pct}%</span>}
                </button>
              );
            })}
          </div>
        ...
        <div className="flex flex-col">
          {ACTIVITY.map((a, i) => (
            <div key={i} className="flex items-center justify-between py-2 border-b border-white/[0.04]">
              ...
              <span className="text-[10px] text-text-muted/50 whitespace-nowrap ml-2">{a.time}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="animate-fade-in mb-6" style={{ animationDelay: "0.4s" }}>
        ...
        <div className="card mb-2">
          ...
          <div className="space-y-1.5">
            ...
            ].map((s, i) => <ScoringRow key={i} label={s.l} pts={s.p} accent={s.p >= 50} />)}
          </div>
        </div>
      </div>
    </div>
...
function ScoringRow({ label, pts, note, accent }: { label: string; pts: number; note?: string; accent?: boolean }) {
  return (
    <div className="flex items-center justify-between py-1">
      ...
      <span className={`font-display text-sm font-bold ml-2 ${accent ? "text-gold-light" : "text-text-primary"}`}>
