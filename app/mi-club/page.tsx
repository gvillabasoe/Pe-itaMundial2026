...
function TabPartidos() {
  const sample = Object.entries(GROUPS).slice(0, 3).flatMap(([g, teams]) => [
    { group: g, home: teams[0], away: teams[1], pick: `${Math.floor(Math.random()*3)}-${Math.floor(Math.random()*3)}` },
    { group: g, home: teams[2], away: teams[3], pick: `${Math.floor(Math.random()*3)}-${Math.floor(Math.random()*3)}` },
  ]);
  return (
      ...
      {sample.map((f, i) => (
        <div key={i} className="card !py-2.5 !px-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-display text-sm font-bold text-text-muted bg-bg-2 rounded px-2 py-0.5">{f.pick}</span>
            <div className="flex items-center gap-1">
              <span className="text-[11px]">{f.away}</span>
              <Flag country={f.away} size="sm" />
            </div>
          </div>
          <div className="text-center mt-1"><span className="badge badge-muted">Pendiente</span></div>
        </div>
      ))}
    </div>
  );
    ...
    <div className="flex flex-col gap-2.5">
      ...
      <div className="card !p-4 text-center !border-gold/15 bg-gold/[0.03]">
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <div className="text-center"><Flag country="Francia" /><p className="text-[11px] mt-0.5">Francia</p></div>
        </div>
