...
import { LogOut, Eye, EyeOff, User, Star } from "lucide-react";
  ...
  return (
    <div className="px-4 flex items-center justify-center min-h-[80vh]">
      <div className="card w-full max-w-[360px] !p-7 text-center bg-gradient-to-b from-bg-4 to-bg-2 animate-fade-in">
        ...
        <h2 className="font-display text-[22px] font-extrabold text-text-warm mb-0.5">
          Mi Club
        </h2>
        <p className="text-xs text-text-muted mb-6">Acceso privado</p>
        <div className="text-left mb-3">
          <label className="text-[11px] text-text-muted mb-1 block" htmlFor="username">
            @usuario
          </label>
          <input
            id="username"
        ...
        <div className="text-left mb-4">
          <label className="text-[11px] text-text-muted mb-1 block" htmlFor="pwd">
            Contraseña
          </label>
          <div className="relative">
            ...
            <button
              ...
              className="absolute right-2.5 top-1/2 -translate-y-1/2 bg-transparent border-none cursor-pointer text-text-muted"
            >
        ...
        <p className="text-[10px] text-text-muted mt-4 opacity-50">
          Prueba: Carlos_M / cualquier contraseña
        </p>
      </div>
    </div>
  );
    ...
    <div className="px-4 pt-4 max-w-[640px] mx-auto">
      <div className="flex items-center gap-3 mb-4 animate-fade-in">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="font-display text-2xl font-extrabold text-text-warm leading-tight">
            Mi Club
          </h1>
      ...
      <div className="card flex items-center gap-3 mb-3 animate-fade-in">
        <div className="w-10 h-10 rounded-[10px] bg-accent-participante/10 flex items-center justify-center">
          <User size={20} className="text-accent-participante" aria-hidden="true" />
        </div>
      ...
      {userTeams.length > 1 && (
        <div className="flex gap-1.5 mb-3 overflow-x-auto">
          {userTeams.map((t, i) => (
            <button
              key={t.id}
              className={`pill ${activeTeamIdx === i ? "active" : ""}`}
      ...
      {team && (
        <>
          <div className="card card-glow mb-2 text-center !py-5 bg-gradient-to-br from-bg-4 to-bg-2 !border-gold/10 animate-fade-in">
            ...
            <div className="flex items-center justify-center gap-1 mb-2">
              ...
              <span className="text-[11px] text-text-muted">
                Campeón: {team.championPick}
              </span>
            </div>
            <p className="font-display text-[40px] font-black text-gold-light">
              {team.totalPoints}
            </p>
            <span className="badge badge-gold mt-1">
              #{team.currentRank} de {PARTICIPANTS.length}
            </span>
          </div>
          ...
          <div className="animate-fade-in">
            ...
            {activeTab === "favoritos" && (
              <TabFavoritos favorites={favorites} toggleFavorite={toggleFavorite} />
            )}
          </div>
        </>
      )}
  ...
  return (
    <div>
      {[
        ...
        <div
          key={i}
          className="flex justify-between py-2.5 border-b border-text-muted/10"
        >
          <span
            className={`text-sm ${
              it.hl ? "text-text-warm font-semibold" : "text-text-muted"
            }`}
          >
            {it.label}
          </span>
          <span
            className={`font-display text-sm ${
              it.hl ? "font-extrabold text-gold-light" : "font-semibold"
            }`}
          >
            {it.val}
          </span>
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex gap-1 overflow-x-auto pb-1 mb-1">
        {Object.keys(GROUPS).map((g) => (
          <button
            key={g}
            className={`pill !px-2.5 !py-1 ${selGroup === g ? "active" : ""}`}
      ...
      {gFixtures.map((f) => {
        ...
        return (
          <div
            ...
            style={{
              borderLeft: isD ? "3px solid #DFBE38" : "3px solid transparent",
            }}
          >
            <div className="flex items-center gap-1.5 mb-1">
              ...
              {isD && (
                <span className="badge badge-amber text-[8px] !py-0 !px-1.5">
                  DOBLE
                </span>
              )}
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                ...
                <span className="text-[11px]">{f.homeTeam}</span>
              </div>
              {pick ? (
              ...
              <div className="flex items-center gap-1">
                <span className="text-[11px]">{f.awayTeam}</span>
            ...
            <div className="mt-2 flex flex-col gap-1">
              {userOrder.map((t, i) => (
                <div key={t} className="flex items-center gap-1.5">
                  <span className="text-[11px] font-bold text-text-muted w-3.5">
                    {i + 1}
                  </span>
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {KNOCKOUT_ROUND_DEFS.map((rd) => {
        ...
        return (
          <div key={rd.key}>
            <h4 className="font-display text-sm font-bold text-text-muted mb-1.5">
              {rd.name}{" "}
              <span className="text-[10px] font-normal">({rd.pts} pts)</span>
            </h4>
            <div className="flex flex-wrap gap-1">
              {picks.map((p, pi) => (
                <div
                  key={pi}
                  className="card !py-1.5 !px-2.5 flex items-center gap-1.5"
                >
                  <Flag country={p.country} size="sm" />
                  <span className="text-[11px]">{p.country}</span>
                  <span className="badge badge-muted text-[9px]">Pendiente</span>
                </div>
              ))}
              {!picks.length && (
                <span className="text-[11px] text-text-muted">Sin picks</span>
              )}
            </div>
          </div>
        );
      ...
      <div className="card !p-4 text-center !border-gold/15 bg-gold/[0.03]">
        <p className="text-[11px] text-gold font-semibold mb-2">
          Final pronosticada
        </p>
        <div className="flex items-center justify-center gap-3">
          ...
          <div className="text-center">
            ...
            <p className="text-[11px] mt-0.5">{team.runnerUpPick}</p>
          </div>
        </div>
        <p className="text-xs font-semibold text-gold-light mt-2">
          Campeón: {team.championPick}
        </p>
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col gap-1">
      {items.map((s, i) => (
        <div
          key={i}
          className="card !py-2.5 !px-3 flex items-center justify-between"
        >
          <div>
            ...
            <p className="text-sm font-semibold mt-0.5">
              {s.isC ? <CountryWithFlag country={s.val as string} /> : s.val}
            </p>
          </div>
...
function TabFavoritos({
  ...
  if (!favTeams.length)
    return (
      <EmptyState
        text="Aún no tienes favoritos. Márcalos desde la Clasificación."
        icon={Star}
      />
    );
  return (
    <div className="flex flex-col gap-1">
      {favTeams.map((p) => (
        <div
          key={p.id}
          className="card !py-2.5 !px-3 flex items-center gap-2.5"
        >
          <span className="font-display text-base font-extrabold text-text-muted min-w-[28px] text-center">
            #{p.currentRank}
          </span>
          <div className="flex-1 min-w-0">
