            ...
            return (
              <div key={g} className="card !p-3">
                ...
                <div className="mt-2">
                  ...
                  {rows.map((r: any) => (
                    <div key={r.pos} className="grid grid-cols-[1fr_auto_1fr] gap-1 items-center py-1 border-t border-[rgb(var(--divider)/0.06)]">
...
function StatBox({ label, val, color }: { label: string; val: string; color?: string }) {
  return (
    <div className="p-2.5 bg-bg-2 rounded-lg text-center">
      ...
      <p className="font-display text-[22px] font-extrabold" style={{ color: color || "rgb(var(--text-primary))" }}>{val}</p>
    </div>
  );
}
